...
- [x] Classify fake-provider workflow tests as integration tests, never E2E tests. Evidence: `TestIntegrationFakeProviderRepairNormalizationAndAxes`, `TestIntegrationG008ProviderRuntimeCapturesRepairArtifactsDeterministically`, and `TestIntegrationG008RealCompositionApplicationChildWorkflows`; the only `TestE2E` symbol is the build-tagged actual-provider gate.
...
- [ ] Restore and exercise the executable full-workflow E2E after the focused three-lane gate passes: Config v2 init/config/doctor, six-role review, followup, three-family delta, exact rerun, and recomposed rerun with schema-valid P2, lineage, assignment, and exit/artifact consistency assertions.
