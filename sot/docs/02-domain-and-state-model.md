...
The role vocabulary and order are fixed: `logic`, `security`, `maintainability`, `product`, `documentation`, `testing`. A project enables a canonical two-to-six-role subset and must enable `logic` and `security`; that is the project-configuration floor. It is not an implicit per-run union. An omitted root-review `--roles` selects every enabled project role, while an explicit `--roles` selects exactly the requested nonempty enabled subset and may omit `logic` and `security`. When either floor role is selected it is always required. `review.required_roles` may mark other enabled roles required when they are selected, but never selects an absent role.
