...
Decisions D-030 through D-040 remain normative: four native platform cells are evidence-only, only `kimi`, `zcode`, and `agy` are G0 provider-probe families, ordinary provider support is family-and-capability based, six core roles are fixed and artist is declaration-gated to UI projects, severity direction is strict, outcomes are independent, retention is deterministic, trust and assignment are fail-closed, and prompt/evidence provenance is byte- and identity-bound.
