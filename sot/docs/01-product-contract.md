...
The G0 approval contract remains a one-way, session-bound sequence. G001 completed it before product implementation. G008 through G011 receipts and live workflow timings are historical evidence rather than current release predicates. Historical qualification evidence does not constrain ordinary family support to a recorded version or SHA-256, broaden support beyond the allowlisted families, or make a future platform release-eligible. G012 closure requires deterministic product acceptance, one non-skipping live capability certification for each supported provider family, and exact release-binary root/child production execution through `make test`. No release asset is authorized by technical readiness alone.
