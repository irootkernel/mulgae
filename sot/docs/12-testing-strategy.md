...
| End-to-end smoke tests | Current production workflow | Required actual Kimi, ZCode, and AGY |

Unit and integration CI do not require provider credentials or network access. The final G010 `make test` release gate deliberately requires the installed actual providers, their native authentication, and available provider service access.
...
G0 contract validators and G001–G009 evidence remain historical. G010 supersedes their use as a current production closure mechanism: only the race unit/integration suites and non-skipping real-provider E2E reached through the final `make test` can establish current release readiness. The G0 validator set remains exact and complete:
...
Fake-provider workflow tests are integration tests. Unit and integration suites run independently with the race detector. G010 E2E builds the current KAR candidate and invokes the actual Kimi, ZCode, and AGY binaries with native authentication; missing prerequisites fail rather than skip. Before the product workflow, the gate runs the installed AGY native-auth probe without opt-in skipping and verifies the installed-user HOME identity, disposable namespaces, immutable prompt-file CWD, safe argv, minimum version, auth/settings non-mutation across KAR setup and drain, and terminal process-group lifecycle.

The product workflow runs one combined review of an immutable committed `HEAD^...HEAD` fixture containing six concurrent primary lanes: `logic=kimi-logic`, `security=zcode-security`, `maintainability=zcode-maintainability`, `product=zcode-product`, `documentation=agy-documentation`, and `testing=zcode-testing`, with `max_active_lanes=6` and six distinct provider-role instances. Its limited-trust objective is role-neutral: it identifies `head` as the current evidence side for this committed target and reinforces the provider JSON contract, but never prescribes another role's conclusion or exact answer. The fixture plants an explicit traversal for the required security finding while other roles may truthfully return findings or no findings.

Provider health and product recovery are separate release predicates. Provider health reads the root run's private runtime log and requires all 12 role-specific primary/fallback candidates to have direct native qualification observations whose final outcome is `qualified`, followed by exactly one `qualification_succeeded`; role fallback cannot hide a missing or rejected qualification. Product recovery requires every configured primary attempt and its initial native process observation, plus a nonempty common intersection across the six initial primary process intervals. Each role must publish a schema-valid P2 outcome selected either from its configured primary after at most one repair or, only after a terminal primary failure, from its exact configured fallback after at most one repair. A successful primary followed by fallback, an unconfigured fallback, duplicate attempts, or an exhausted role fails the gate. The planted security finding must bind the provider selected for the security outcome, so its followup lineage remains correct when fallback is exercised.

The harness may issue at most three whole-review commands after a permitted non-P2 provider-execution or readiness-unverified stop, an explicitly retryable `provider_qualification_failed`, a P2 that lacks a complete recoverable role outcome or required security finding, or a primary run without the required six-way overlap. A successful bounded repair or configured fallback P2 is accepted immediately rather than retried. The harness may retry a non-P2 security-class qualification stop only when every returned reason is the non-retryable `provider_qualification_failed`, every reason carries a safe diagnostic URI, and every referenced terminal status records exactly `provider_output_frame_missing`; this bounded operator-level repetition does not weaken KAR's security classification, fallback prohibition, or command-envelope retry authority. Missing or malformed qualification/process diagnostics, mixed reasons, or any other security cause fail immediately. A Kimi qualification login requirement may trigger the single bounded native login-and-requalification flow; a repeated Kimi requirement, ZCode/AGY login requirement, configuration stop, artifact stop, and every security stop outside the exact missing-frame qualification allowance are never retried.

After the accepted root P2, the same executable scenario runs followup, a three-family delta, exact rerun, and recomposed rerun with schema-valid P2 and lineage/assignment/exit/artifact assertions; any missing qualification, invocation, lineage, or publication fails the gate. Followup and exact rerun remain source-provider bound without fallback. Followup alone may use one same-provider repair for structurally repairable provider output; the repaired P2 must retain both invocations under one attempt. Delta and recomposed rerun use current Config v2 assignments. Every child command opens and finalizes runtime diagnostics, and terminal non-P2 assertions require the typed reason and installed diagnostic URI rather than accepting a generic artifact projection. Deterministic injected integration tests retain authority for the fallback eligibility/prohibition matrix because destructive network or credential sabotage is not an E2E authority. The UI-only artist role remains covered by its Playwright-backed integration scenario and is not added to the six-core-role actual-provider root. `make test` remains the sole technical release-ready gate and runs `test-prepare`, `test-unit`, `test-int`, and `test-e2e` in order.

## 14. Runtime Diagnostic Verification

Diagnostic model and port tests reject invalid identifiers, unknown codes, unsafe values, non-UTC timestamps, decreasing elapsed time, non-monotonic sequence, aliased mutable input, and inconsistent status identity. Filesystem tests cover anchored-root creation, `0700`/`0600` modes, symlink/path escape, permissions, secret detection, overflow/drop metadata, JSONL cap and mandatory tail reserve, partial append recovery, crash and fsync failures, atomic status replacement, concurrent append under the race detector, writer failure classification, and exactly-once finalize. D-E01 gated both generators, `make test-prepare`, and race tests for domain, ports, filesystem, and architecture; actual-provider E2E was non-gating only during the diagnostics initiative and became mandatory when its evidence was handed to G010-T05.
