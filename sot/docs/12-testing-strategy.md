...
Fake-provider workflow tests are integration tests. Unit and integration suites run independently with the race detector. G010 E2E builds the current KAR candidate and invokes the actual Kimi, ZCode, and AGY binaries with native authentication; missing prerequisites fail rather than skip. T05 stabilization first runs one focused combined review of an immutable committed `HEAD^...HEAD` fixture containing three independent concurrent primary lanes: `logic=kimi`, `security=zcode`, and `documentation=agy`, with `max_active_lanes=3` and three distinct provider instances. The fixture oracle fixes the valid-negative logic/documentation results and the planted security finding's exact `head` path, line range, and quote. Because current native providers can fail transiently, the gate may issue at most three whole-review commands only after a non-P2 internal/readiness `provider_execution_failed` result or a P2 run that used a non-primary outcome. The accepted run must still publish schema-valid P2 from exactly one successful initial primary invocation per role with no repair or fallback. Its truthful role outcome may be `completed` or `degraded`, but provider binding, primary selection, successful attempt state, and one-invocation cardinality are mandatory. `provider_login_required`, security, configuration, and artifact stops are never retried, and three non-primary runs fail the gate. A native `login_required` must instead fail the gate before P2 with `provider_login_required` and the affected provider instance, allowing the user to authenticate and rerun without weakening the gate. The full workflow scenario remains outside the executable E2E set until this focused gate passes; this intermediate state cannot establish release readiness. The final executable E2E set must restore Config v2 init/config/doctor, six-role review, followup, a three-family delta, exact rerun, and recomposed rerun with schema-valid P2 and lineage/assignment/exit/artifact assertions. Deterministic injected integration tests cover primary failure activation of configured fallback because destructive network or credential sabotage is not an E2E authority. `make test` remains the sole technical release-ready gate and runs `test-prepare`, `test-unit`, `test-int`, and `test-e2e` in order; it cannot pass as a release gate while the full workflow scenario is disabled.
