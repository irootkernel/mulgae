...
Unit and integration tests do not require provider credentials or network access. The final G011 `make test` release gate deliberately requires the installed actual providers, their native authentication, and available provider service access.
...
G0 contract validators and G001–G010 evidence remain historical. G011 supersedes their use as a current production closure mechanism: only the complete preparation, race unit/integration, and non-skipping real-provider suites reached through `make test` can establish current release readiness. The G0 validator set remains exact and complete:
...
`test-e2e` builds the current KAR candidate and performs one live capability certification for each supported family: Kimi, ZCode, and AGY. Each certification uses the installed executable and native authentication and verifies the family's production argv and environment boundary, immutable fixture CWD, prompt transport, bounded process lifecycle, output-frame recovery, runtime capability contract, and non-mutation of protected credentials/settings. Missing binaries, launcher state, native authentication, service access, or a valid live response fail rather than skip.

Live certification does not require a provider to invent a finding, cite a predetermined exact line, agree with another role, exercise a fixed role matrix, overlap a fixed number of processes, or reproduce every product workflow. Those are either stochastic assertions or deterministic product behaviors and therefore belong to controlled integration tests. Historical G010 full-workflow timings, retry counts, twelve-route qualification cardinality, six-lane intersection, and planted-finding checks remain historical evidence only.

`make test` is the sole technical release-ready gate and runs `test-prepare`, `test-unit`, `test-int`, and `test-e2e` in order. No required test may silently skip because a tool, browser runtime, executable, credential, or service prerequisite is absent. Release readiness may be recorded only after this command passes on the exact final committed tree and the working tree remains clean.

## 14. Runtime Diagnostic Verification

Diagnostic model and port tests reject invalid identifiers, unknown codes, unsafe values, non-UTC timestamps, decreasing elapsed time, non-monotonic sequence, aliased mutable input, and inconsistent status identity. Filesystem tests cover anchored-root creation, `0700`/`0600` modes, symlink/path escape, permissions, secret detection, overflow/drop metadata, JSONL cap and mandatory tail reserve, partial append recovery, crash and fsync failures, atomic status replacement, concurrent append under the race detector, writer failure classification, and exactly-once finalize. G011 retains these deterministic requirements and the fail-closed actual-provider boundary under the single `make test` gate.
