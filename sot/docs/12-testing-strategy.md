...
| `trace` | `trace-ledger.json`, 85-path catalog | `TRACE_OK`, 17 product commands, no orphan | `E/trace/receipt.json` |
...
| `integrity` | `integrity-cases.json`, catalog, checksums | `INTEGRITY_OK`, 84 payload files, raw32 reducer | `E/integrity/receipt.json` |
| `authority` | `authority-cases.json`, runtime state receipt | `AUTHORITY_OK` | `E/authority/receipt.json` |
| `checksums-generate` | catalog and SOT payload | `CHECKSUMS_OK`, 84 payload files | `E/checksums-generate/receipt.json` |
| `checksums-verify` | checksums and SOT payload | `CHECKSUMS_OK`, 84 payload files | `E/checksums-verify/receipt.json` |
...
Prompt fixtures exercise canonical frames, declared length, section and stdin hashes, malformed/truncated input, fresh execution identity, and exact replay. Evidence fixtures cover source/current identity separation, stale evidence, path traversal, range errors, spoofing, hash mismatch, and missing immutable bytes. Publication fixtures cover all persisted states, P0/P1/P2 recovery, the ten named cross-boundary observations, and immutable `corrupt` diagnostics. Cleanup fixtures cover retained seeds, transitive ancestors, corrupt graph protection, separate age and size sets, fixed epoch, plan hash, tombstone restart, and stale-plan rejection. Integrity fixtures cover the 85-path catalog, 84-file checksum payload, `KAR-SOT-PAYLOAD-ROOT/1` domain, UTF-8 bytewise sorting, NUL plus raw32 digest records, and the empty-set vector. Authority fixtures cover runtime-only approvals, candidate reviews, forward CAS, delete-ref rollback CAS, post-verification, and the independent implementation approval boundary.
...
The authoritative current pair list is the 25 schema/example relationships in `examples/g0-file-catalog.v1.valid.json`. Validation must compile every schema as Draft 2020-12 where applicable and validate each paired example. Released v1 pairs remain compatibility cases; v2 provider/platform evidence is the only readiness authority. Negative cases remove or corrupt required fields and must fail.
...
Fake-provider workflow tests are integration tests. Unit and integration suites run independently with the race detector. G010 E2E builds the current KAR candidate and invokes the actual Kimi, ZCode, and AGY binaries with native authentication; missing prerequisites fail rather than skip. The focused gate runs one combined review of an immutable committed `HEAD^...HEAD` fixture containing six concurrent primary lanes: `logic=kimi-logic`, `security=zcode-security`, `maintainability=zcode-maintainability`, `product=zcode-product`, `documentation=agy-documentation`, and `testing=zcode-testing`, with `max_active_lanes=6` and six distinct provider-role instances. Its limited-trust objective is role-neutral: it identifies `head` as the current evidence side for this committed target and reinforces the provider JSON contract, but never prescribes another role's conclusion or exact answer. The fixture plants an explicit traversal for the required security finding while other roles may truthfully return findings or no findings. The accepted run must publish schema-valid P2 from exactly one successful initial primary invocation per role with no repair or fallback, and the six process-observation intervals recorded in invocation diagnostics must have a nonempty common intersection. Because current native providers can fail transiently, the gate may issue at most three whole-review commands after a permitted non-P2 provider-execution, readiness-unverified stop, explicitly retryable `provider_qualification_failed`, P2 non-primary outcome, or primary run without the required six-way overlap. Missing or malformed process diagnostics fail immediately. A Kimi qualification login requirement may trigger the single bounded native login-and-requalification flow; a repeated Kimi requirement, ZCode/AGY login requirement, security, configuration, or artifact stop is never retried. The full workflow scenario remains outside the executable E2E set until explicitly restored and therefore this focused gate alone cannot establish release readiness. The final executable E2E set must restore Config v2 init/config/doctor, six-role review, followup, a three-family delta, exact rerun, and recomposed rerun with schema-valid P2 and lineage/assignment/exit/artifact assertions. Deterministic injected integration tests cover primary failure activation of configured fallback because destructive network or credential sabotage is not an E2E authority. `make test` remains the sole technical release-ready gate and runs `test-prepare`, `test-unit`, `test-int`, and `test-e2e` in order; it cannot pass as a release gate while the full workflow scenario is disabled.

## 14. Runtime Diagnostic Verification

Diagnostic model and port tests reject invalid identifiers, unknown codes, unsafe values, non-UTC timestamps, decreasing elapsed time, non-monotonic sequence, aliased mutable input, and inconsistent status identity. Filesystem tests cover anchored-root creation, `0700`/`0600` modes, symlink/path escape, permissions, secret detection, overflow/drop metadata, JSONL cap and mandatory tail reserve, partial append recovery, crash and fsync failures, atomic status replacement, concurrent append under the race detector, writer failure classification, and exactly-once finalize. D-E01 gates both generators, `make test-prepare`, and race tests for domain, ports, filesystem, and architecture; actual-provider E2E is non-gating until the diagnostics initiative hands its evidence to G010-T05.
