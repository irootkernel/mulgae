...
G0 validation produces contract and evidence receipts only; it is not product testing and does not claim a provider or platform PASS. Implementation Readiness remains `CONDITIONAL` and External Contract Readiness remains `UNVERIFIED`. The G0 validator set is exact and complete:
