...
Fake-provider workflow tests are integration tests. Unit and integration suites run independently with the race detector. G010 E2E builds the current KAR candidate and invokes the actual Kimi, ZCode, and AGY binaries with native authentication; missing prerequisites fail rather than skip. It exercises Config v2 init/config/doctor, six-role review, followup, a three-family delta, exact rerun, and recomposed rerun. Deterministic injected integration tests cover primary failure activation of configured fallback because destructive network or credential sabotage is not an E2E authority. `make test` is the sole technical release-ready gate and runs `test-prepare`, `test-unit`, `test-int`, and `test-e2e` in order.
