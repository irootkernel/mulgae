...
| Implementation Readiness | Whether product implementation is authorized; it remains `CONDITIONAL` until G0 completes and a separate approval exists |
| External Contract Readiness | Whether required provider tuples and native platform cells have passed; it remains `UNVERIFIED` until complete evidence exists |
