...
| TASK-002 | Planned | Add a session-local MCP invocation registry that owns review execution independently of an individual wait request. | Prove monotonic lifecycle state, single execution ownership, bounded shutdown drain, and no restart recovery. |
...
deferred final goal of this epic. The epic remains active while TASK-002 and
TASK-003 are planned. Completing TASK-003 delivers the supported custom
