...
const artistReviewSchema = "https://kar.local/schemas/kar-review-artifact.v3.schema.json"
...
func TestIntegrationArtistHomepageWorkspaceReview(t *testing.T) {
	...
	configBytes, err := os.ReadFile(filepath.Join(project, ".kar", "config.yaml"))
	if err != nil {
		t.Fatal(err)
	}
...
func assertArtistFinalArtifact(t *testing.T, contents []byte, screenshotSHA string, bbox artistE2EBBox) {
	...
	if artifact.SchemaVersion != "kar-review-artifact.v3" || len(artifact.Findings) != 1 ||
...
func assertArtistCapturedArchive(t *testing.T, project, sessionID, runID, referenceSHA, currentSHA string) {
	t.Helper()
	path := filepath.Join(project, ".kar", sessionID, runID, "target", "captured-review.json")
	contents, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
