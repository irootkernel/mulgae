...
func (service deferredFollowupRunService) StartFollowupRun(ctx context.Context, request appfollowup.Request) (_ kar.StartedRun, err error) {
	...
	if err != nil {
		return kar.StartedRun{}, abortCaptured(captured, err)
	}
	authority, err := authorityFactory.NewQualifiedRun(ctx, captured, selection)
	if err != nil {
		return kar.StartedRun{}, abortCapturedAfterAuthorityError(captured, err)
	}
	completed := false
	...
	result, err := kar.NewFollowupRunService(workflow).StartFollowupRun(ctx, request)
	if err != nil {
		return kar.StartedRun{}, err
	}
...
func (service deferredDeltaRunService) StartDeltaRun(ctx context.Context, request appdelta.StartRequest) (_ kar.StartedRun, err error) {
	graph, err := service.composer.graph(ctx)
	if err != nil {
		return kar.StartedRun{}, err
	}
	...
	if len(roles) == 0 {
		...
		if sourceErr != nil {
			return kar.StartedRun{}, sourceErr
		}
	...
	result, err := kar.NewDeltaRunService(workflow).StartDeltaRun(ctx, request)
	if err != nil {
		return kar.StartedRun{}, err
	}
	childRunID, completed = result.RunID, true
...
func (service deferredRerunService) StartRerun(ctx context.Context, request appreplay.Request) (_ kar.StartedRun, err error) {
	graph, err := service.composer.graph(ctx)
	if err != nil {
		return kar.StartedRun{}, err
	}
	...
	result, err := kar.NewRerunService(workflow).StartRerun(ctx, request)
	if err != nil {
		return kar.StartedRun{}, err
	}
	childRunID, completed = result.RunID, true
...
func (graph *productionRuntimeGraph) childExecutor(ctx context.Context, artifactRoot ports.AnchoredRoot, captured reviewrun.CapturedRunInput, selection reviewrun.RunSelection, authority reviewrun.RunAuthority, sources *kar.G008Sources) (*childrun.Executor, []review.Assignment, error) {
	...
	trustedPrompts, err := kar.NewG008RuntimePromptSource(sources, promptSource, promptSource, promptSource)
	if err != nil {
		return nil, nil, err
	}
...
var _ kar.FollowupRunService = deferredFollowupRunService{}
var _ kar.DeltaRunService = deferredDeltaRunService{}
var _ kar.RerunService = deferredRerunService{}
