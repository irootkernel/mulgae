...
func TestInstallConfigRejectsTemporaryByteMutationBeforeInstall(t *testing.T) {
	...
	if _, statErr := os.Lstat(filepath.Join(rootPath, ".mulgae", "config.yaml")); !errors.Is(statErr, os.ErrNotExist) {
...
func TestInstallConfigClassifiesCancellationWithConcurrentDestinationAsCollision(t *testing.T) {
	...
	operations.write = func(fd int, data []byte) (int, error) {
		...
		if writeErr == nil && !created {
			created = true
			if err := os.WriteFile(filepath.Join(rootPath, ".mulgae", "config.yaml"), []byte("external\n"), 0o600); err != nil {
				return written, err
			}
			cancel()
		}
	...
	entries, readErr := os.ReadDir(filepath.Join(rootPath, ".mulgae"))
...
func TestInstallConfigRejectsPreparedDirectorySubstitutionBeforeTempCreation(t *testing.T) {
	...
	if err := os.Rename(filepath.Join(rootPath, ".mulgae"), filepath.Join(rootPath, ".mulgae-original")); err != nil {
		t.Fatal(err)
	}
	if err := os.Mkdir(filepath.Join(rootPath, ".mulgae"), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(rootPath, ".mulgae", "config.yaml"), []byte("replacement\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	receipt, err := writer.InstallConfig(context.Background(), root, prepared, []byte("version: 1\n"))
	if err == nil || receipt.Installed() {
...
func TestPrepareConfigDirectoryRootSyncReportsConcurrentDestination(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			contents, err := os.ReadFile(filepath.Join(rootPath, ".mulgae", "config.yaml"))
...
func TestInstallConfigReportsNamespaceLossAsInstalledNotObserved(t *testing.T) {
	...
	operations.fsync = func(fd int) error {
		fsyncCalls++
		if fsyncCalls == 2 {
			if err := os.Rename(filepath.Join(rootPath, ".mulgae"), filepath.Join(rootPath, ".mulgae-installed")); err != nil {
				return err
			}
			if err := os.Mkdir(filepath.Join(rootPath, ".mulgae"), 0o700); err != nil {
				return err
	...
	if _, err := os.Lstat(filepath.Join(rootPath, ".mulgae", "config.yaml")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("visible replacement contains config: %v", err)
	}
	contents, err := os.ReadFile(filepath.Join(rootPath, ".mulgae-installed", "config.yaml"))
