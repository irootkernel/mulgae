...
| Implementation Readiness | Current production implementation status. It is `RELEASE_READY`: G013 executes the login-recovering exact release-binary actual-provider root/child workflow before all three live capability certifications on the exact committed tree. Historical gate passes do not establish current readiness. Release CI, asset creation, and publication remain separately approved actions. |
| External Contract Readiness | Whether allowlisted provider families satisfy current runtime capability and security contracts and native platform cells satisfy their declared support contracts. G013 requires both the exact-binary production composition, including bounded Kimi login recovery, and the subsequent Kimi, ZCode, and AGY capability boundary; a missing required capability or workflow result fails closed. Only `darwin-arm64` is supported; future platforms remain unsupported. Historical exact-tuple evidence remains diagnostic provenance, not ordinary-use version or SHA-256 gating. |
