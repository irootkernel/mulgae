...
|---|---|
| Global, home, or XDG configuration | Never read. `<canonical-project-root>/.mulgae/config.yaml` is the sole configuration authority. |
...
| Versioned `.mulgae/context.md` or `.mulgae/prompts/` | Forbidden as authority. The entire `.mulgae/` namespace is private operator and runtime state and should be ignored by Git. |
