...
|---|---|---:|
| Mulgae | Deterministic execution, identity, policy, validation, or artifact metadata | No |
...
|---|---|---|---:|
| `/content_verdict` | Mulgae assessment aggregator | Deterministic aggregation of normalized findings: `no_findings`, `findings_present`, or `request_changes` | No |
...
| `/ci_decision` | Mulgae CI policy projector | `pass` or `fail`, projected from valid content, coverage, and configured policy | No |
| `/ci_reason_codes` | Mulgae CI policy projector | Deterministic, bounded reasons for the CI projection | No |
...
|---|---|---|---|
| Source identity | `session_id`, `run_id`, `review_id`, `finding_id`, `source_target_sha256`, `source_excerpt_sha256` | Mulgae source artifact and evidence reducer | `source_excerpt_sha256` identifies only the immutable original source excerpt; provider repair is prohibited and it cannot substitute for current evidence |
| Current evidence | `target_sha256`, `current_excerpt_sha256`, `path`, `line_start`, `line_end`, `side`, `quote`, `verification` | Mulgae injects target identity; provider claims path/range/quote; Mulgae evidence reducer computes and owns `current_excerpt_sha256`, acceptance, and final serialization | `current_excerpt_sha256` identifies the newly verified current excerpt and controls indexed excerpt verification and order; it must not be conflated with or fall back to `source.source_excerpt_sha256`. `side` is `base`, `head`, or `worktree`; provider output is `claimed`, while final state is `verified`, `stale`, `invalid`, or `unverifiable` |
| Current verified projection | All current-evidence fields plus `verification=verified` | Mulgae evidence reducer | Requires exact current target, range, quote, `current_excerpt_sha256`, and indexed excerpt order match; source identity alone can never satisfy it |

The v2 provider schemas serialize a Mulgae-normalized result envelope. Mulgae injects the current target digest for every review and injects immutable source identity only for source-bearing followup, delta, rerun, or equivalent modes; root review omits `source`. The provider owns finding text and the current path/range/quote claim only; its wire value is always `verification=claimed`. The evidence reducer owns every transition to `verified`, `stale`, `invalid`, or `unverifiable` and serializes that state only in validation/final artifacts.
...
Schema: [mulgae-provider-review-output.v1.schema.json](../schemas/mulgae-provider-review-output.v1.schema.json)

| JSON path | Mandatory | Meaningful non-empty value | Owner | Repairable | Additional validation |
|---|---:|---:|---|---:|---|
| `/schema_version` | Yes | Yes | Provider, fixed constant | Yes if missing | Must equal `mulgae-provider-review-output.v1` |
...
| `/findings/*/evidence` | For each finding | At least one item | Provider | Yes if missing and relevant target context is supplied | Mulgae verifies path, lines, side, and quote |
...
Schema: [mulgae-provider-followup-output.v1.schema.json](../schemas/mulgae-provider-followup-output.v1.schema.json)
...
Mulgae supplies the source finding reference in the prompt and records it in the final artifact. The provider does not invent or rewrite that reference.

## 7. Final Review Artifact (v1 Compatibility)

Schema: [mulgae-review-artifact.v1.schema.json](../schemas/mulgae-review-artifact.v1.schema.json)

All mandatory fields in the final artifact are Mulgae-owned or deterministically normalized. None are AI-repairable at this stage.

| JSON path | Owner | Source |
|---|---|---|
| `/schema_version` | Mulgae publisher | Configured final schema |
| `/session_id` | Mulgae ID generator | Session creation |
| `/run_id` | Mulgae ID generator | Run creation |
| `/review_id` | Mulgae ID generator | Generated after final validation |
| `/run_type` | Mulgae application request | CLI workflow |
| `/created_at` | Mulgae clock | Publication time |
| `/mulgae` | Mulgae build metadata | Running binary |
| `/lineage` | Mulgae coordinator | Source and parent run references |
...
| `/assessment` | Mulgae policy and aggregation | Findings, coverage, and threshold policy |
...
If any of these values cannot be produced, publication fails. Mulgae must not ask a provider to fill the gap.

## 8. Run Manifest (v1 Compatibility)

Schema: [mulgae-run-manifest.v1.schema.json](../schemas/mulgae-run-manifest.v1.schema.json)

| Field group | Owner | Mutable before sealing | AI repairable |
|---|---|---:|---:|
| Session, run, type, timestamps | Mulgae | Limited atomic replacement | No |
...
| Missing system-owned metadata | None | No | No, Mulgae failure |
...
```text
final schema passes
all system-owned fields were generated by Mulgae
