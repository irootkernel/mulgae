...
It does not decide content, coverage, publication, or CI outcomes; it reports immutable typed attempt results to the central coordinator. G010 through G012 are historical. G013 is release-ready after the deterministic, login-recovering exact release-binary workflow, and subsequent live family-certification gate defined by SOT 1.15.0 passed on the exact committed tree.
...
Distinct role keys allow all selected ZCode processes to overlap against the
same read-only captured workspace.
Configuration and doctor inventory remain family-level; manifests and runtime
diagnostics retain the exact selected instance.
...
The coordinator does not invent family preferences. Config v1 owns each
...
1. At most one active child-process invocation exists per `concurrency_key` in one Mulgae process.
...
In-process goroutines do not prevent two separate `mulgae` processes from invoking the same provider concurrently.

Recommended default for local interactive use:

```text
$XDG_RUNTIME_DIR/mulgae/locks/{normalized_concurrency_key}.lock
```

Fallback when `XDG_RUNTIME_DIR` is unavailable:

```text
${TMPDIR}/mulgae-{uid}/locks/{normalized_concurrency_key}.lock
```
...
Workspace access is closed to `none|readonly_snapshot`; `mulgae init` writes the required value `none`. `readonly_snapshot` remains an immutable read-only snapshot; native `@file` transport refers only to captured material. Kimi and ZCode receive an isolated `HOME` with their projected credentials; neither mode may expose the live project root. AGY is the narrow macOS authentication exception defined in §8.1, not a workspace-access exception.
...
separate readable regular-file identity: Mulgae descriptor-opens, hashes, and
...
On macOS, AGY authentication is bound to the installed user's native `HOME` and Keychain context. Mulgae explicitly captures that installed-user `HOME` and inode-revalidates it immediately before AGY spawn; AGY alone receives it for authentication. Mulgae must not substitute a synthetic `HOME`, copy OAuth or installation files into one, or project AGY credentials. Mulgae's namespace setup, policy handling, and cleanup must never write, overwrite, zero, or unlink the user's AGY authentication or settings files. AGY itself may perform its normal Keychain/profile refresh while running; Mulgae does not misrepresent that provider-owned behavior as namespace-owned mutation or cleanup.

The authentication `HOME` is distinct from the descriptor-bound immutable review CWD, which remains the captured snapshot. Mulgae owns AGY's XDG, cache, temporary, and scratch namespaces; they do not relocate or authorize mutation of the installed user's AGY state. Kimi and ZCode retain isolated `HOME` directories plus credential projection.

Mulgae does not install an AGY `settings.json` policy. Its enforceable AGY controls are direct argv with `--sandbox`, the exact immutable-snapshot `--add-dir`, `--mode plan`, bounded time and output, and post-output process-group `SIGTERM` followed by `SIGKILL` when required. A provider that handles Mulgae's accepted post-output `SIGTERM` and exits nonzero does not invalidate the exact stable JSON frame that preceded and authorized that signal; a nonzero natural exit without that bound signal remains a failure. G010 did not claim this boundary complete until the non-skipping actual-AGY E2E and final `make test` both succeeded on 2026-07-26.
...
If no configured eligible fallback exists, `fallback_scheduled=false` and the role is immediately exhausted despite `fallback=allowed`. Any exhausted selected role gives `coverage_status=incomplete`, preserves valid content from all roles, fails the run, and exits `4`. `coverage_status=degraded` is reserved for a valid, policy-permitted limited result rather than lane exhaustion.

`login_required` is the explicit user-action exception to the generic `auth` row. During Kimi qualification, Mulgae may invoke the native configured executable once as `kimi login` in the native home and configured Kimi data home, then rebuild isolated namespaces and repeat current qualification once. A failed login or a repeated Kimi login requirement stops fail-closed. ZCode/AGY login requirements and every execution-time login requirement stop immediately, schedule neither repair nor fallback, publish no P2 result, and report affected instances with `provider_login_required`. No condition authorizes provider substitution.

Current qualification probes only the requested roles for which a configured provider family is the selected primary or fallback; it must not probe that provider for unrelated requested roles. The admitted route carries exactly that assignment-scoped role authority. When current qualification rejects one or more configured candidates for an operationally retryable reason, Mulgae must retain the configured provider instance and one closed safe reason code per rejected candidate. If a selected role requires a rejected candidate, planning fails before provider execution and P2 publication with `provider_qualification_failed`; the diagnostic lists the affected instances in canonical family order and never includes native stdout, stderr, paths, credential material, or free-form provider text. Partial qualification must not collapse a missing configured route into an unattributed `readiness_unverified` result.
...
run_total_output_cap =
  Σ selected roles Σ present(primary initial, primary repair, fallback initial, fallback repair)
...
|---|---|---:|---|
| `darwin-arm64` | `required` | Yes | Sole supported release platform; G001 native/local-POSIX evidence and the current G013 exact-tree executable gate establish its required boundary. Prior G009 integrated receipts remain `HISTORICAL_GATE_PASS_NON_PRODUCTION` |
...
`external_contract_readiness` is verified only while the required `darwin-arm64` platform evidence and complete provider readiness conjunction PASS. G001's historical evidence satisfied the platform inputs; G013 must independently verify the current exact-binary production and subsequent capability boundaries. Configuration intent, partial evidence, a future row, or a non-native result still cannot promote readiness.
...
For each array, `argv_json_bytes` are the exact UTF-8 bytes above with no whitespace outside the compact JSON and no trailing LF. Its hash is `SHA-256(ASCII("Mulgae-G0-ARGV/1") || 0x00 || argv_json_bytes)`:
...
Each invocation uses a run-wide positive ordinal and a closed purpose in the path `<ordinal>-<purpose>`; this ordinal is distinct from the invocation identity. Provider stdout and stderr remain separate byte-preserving streams with their already-approved invocation capture caps. They are never merged, timestamped, newline-normalized, or copied into `mulgae-runtime.jsonl`. Process/provider boundaries must eventually preserve the best valid observation with a typed cause, but diagnostics do not change provider assignment, fallback eligibility, native authentication, or repair policy.

`provider_transport_verification_failed` remains the closed parent cause for transport security failures. Qualification and process boundaries retain a more specific closed cause when available: `provider_prompt_file_prestart_identity_failed`, `provider_prompt_file_posttermination_identity_failed`, `provider_transport_receipt_mismatch`, `provider_lifecycle_receipt_invalid`, `provider_output_frame_mismatch`, or `provider_signal_receipt_mismatch`. `provider_output_frame_missing` remains the existing missing-frame cause. These subtypes carry no native text, path, packet content, or credential material and project to the same security stop as the parent cause; they refine local diagnosis only and never make a security failure retryable.
