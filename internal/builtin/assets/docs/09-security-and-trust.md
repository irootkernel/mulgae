...
Mulgae processes repository-controlled files, arbitrary diffs, user objectives, prior AI output, and local provider executables. Any of these may be malformed, adversarial, compromised, or unexpectedly sensitive.
...
| `.mulgae/config.yaml` | Sole operator-local configuration authority after locality admission | Define the configured provider subset and bounded policy |
...
Mulgae may capture repository state before and after execution to detect source
...
Mulgae must not store API keys in local YAML or examples. Provider CLIs own authentication.
...
A provider may transmit the prompt and target to a remote service. `mulgae doctor security` and help must state this clearly. Users must select providers appropriate for their data policy.

## 8. Shared Scan-Before-Write

`mulgae-secure-writer/v1` is mandatory before every newly persisted untrusted copy:
...
Mulgae must report a hash mismatch as artifact corruption. It must not silently regenerate a missing or altered final review from normalized summaries.

## 12. CI Project-Local Authority Posture

CI checks out the target first, then provisions `.mulgae/config.yaml` as private untracked operator state through `mulgae init` or an equivalent secure installation step. The file records absolute provider identities for that CI machine, explicitly records the required `workspace_access: none`, and remains outside repository control. Mulgae re-attests the root, `.mulgae`, configuration, checkout, index, and applicable commits across admission and execution boundaries. Repository content cannot supply a fallback configuration, select a provider command, disable the required role floor, or expand workspace access. Mulgae and provider adapter versions remain pinned by the CI environment.
...
- Mulgae cannot guarantee the behavior of an arbitrary provider executable.
