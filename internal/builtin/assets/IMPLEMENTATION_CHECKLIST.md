# Mulgae Implementation Status Checklist

**Status date:** 2026-07-28

SOT 1.15.0 preserves historical G001–G012 evidence and the runtime-diagnostics contract. G013 corrects the release-gate order so the exact Mulgae workflow can exercise bounded Kimi login recovery before all three fail-closed family capability certifications. G013 is **RELEASE_READY** after the exact final-tree gate passed. Historical evidence and `.gjc/` remain append-only.
...
The checked items through G012 are historical evidence and do not establish current release readiness. The dedicated G013 section is the current completion authority.
...
- [x] Validate the 84-file payload root with the `Mulgae-SOT-PAYLOAD-ROOT/1` domain, UTF-8 bytewise path sort, NUL, raw 32-byte digest, and LF grammar.
...
- [x] Implement `.mulgae/{session_id}/{run_id}/review_{uuidv7}.json` as the only final publication path. See [Artifacts](docs/08-artifacts-lineage-and-storage.md).
...
- [x] Separate AI-owned and Mulgae-owned mandatory fields. See [Field Ownership Matrix](docs/16-field-ownership-matrix.md).
...
- [x] Generate excerpts inside Mulgae after verification.
...
- [x] G010 macOS AGY native-auth boundary: use only the captured and inode-revalidated installed-user `HOME`/Keychain context for AGY authentication; do not create a synthetic AGY `HOME`, project credentials, or copy OAuth or installation files. Mulgae's namespace setup, policy, and cleanup paths must not write, overwrite, zero, or unlink user AGY authentication/settings files; normal provider-owned Keychain/profile refresh may still occur during AGY execution. Preserve the descriptor-bound immutable review CWD and Mulgae-owned XDG/cache/temp/scratch namespaces; enforce `--sandbox`, exact immutable-snapshot `--add-dir`, `--mode plan`, bounded time/output, and post-output `SIGTERM`/`SIGKILL`. AGY's minimum version is 1.1.4. Evidence: installed AGY 1.1.7 native boundary and auth/settings mutation checks passed in the non-skipping final-tree E2E on 2026-07-26.
...
- [x] Admit the installed native account before `mulgae config` exposes an accepted digest.
...
- [x] Historical integrated-gate classification: **HISTORICAL_GATE_PASS_NON_PRODUCTION**. At G009 closure, production `mulgae review` composition was wired but remained **REOPENED_PRODUCTION_REVIEW_INCOMPLETE** pending later verification. G011 later supplied capability-only acceptance but is now historical under G012; this row grants no release-publication authority.
...
- [x] Implement canonical Config v1 and reject every other version without migration or compatibility fallback. Evidence: `TestConfigV1RoleAssignmentsAndFutureVersionRejection`.
...
- [x] Expose only redacted role-family assignments through `mulgae config` and retain strict credential rejection. Evidence: `TestRedactionOmitsExecutableAndNativePaths` and adapter credential tests.
...
- [x] Run production `review`, `delta`, and recomposed `rerun` through current Config v1 primary/fallback assignments. Evidence: shared `productionRuntimeGraph`, deferred child services, and configured planner tests.
...
- [x] Revalidate configuration, provider authority, immutable target, prompt, and credential namespace per CLI process and clean Mulgae-owned temporary state on every terminal path. Evidence: per-command deferred graph composition, packet screening, workspace completion/abort, namespace drain, and private-root cleanup.
...
- [x] Promote SOT 1.12.0 without changing CLI, Config v1, or P2 schemas; separate direct health for all 12 selected qualification candidates from recovery-aware six-role product acceptance. Evidence: `TestLiveQualificationHealthGate`, `TestLiveRecoverableAssignmentGate`, and `TestE2ELiveFullWorkflowAndNoSkipContract`.
...
- [x] Build one current Mulgae candidate and run non-skipping E2E against the actual Kimi, ZCode, and AGY binaries and native authentication. Evidence: the same clean `make test-e2e` built the candidate and completed the focused three-provider gate on 2026-07-22.
...
- [x] Preserve the 84-path/83-payload catalog, 27 schema/example pairs, checksum grammar, and generated embedded catalog after every SOT update. Evidence: both required generators and `test-prepare` passed on the exact updated SOT tree.
...
- [x] Assign deterministic controlled-process tests authority over Config v1, supported roles and provider subsets, committed and dirty targets, qualification planning, scheduling, repair/fallback, root and child workflows, schemas, evidence, diagnostics, publication, recovery, cleanup, and CLI exit projection.
...
- [x] Complete deterministic acceptance coverage, including both committed and dirty targets and the required Playwright-backed artist scenario without prerequisite skipping. Evidence: the seven-subset Config v1 binary matrix; fixed-role and provider-route planner matrices; capture scope and six-lane coordinator integration suites; root and G008 child workflow suites; schema, evidence, diagnostics, publication, recovery, and cleanup suites; `TestIntegrationArtistHomepageWorkspaceReview`; and `TestRequiredArtistPrerequisitesFailClosed`.
- [x] Replace the historical full-workflow live test with the three fail-closed family capability certifications. `make test-e2e` resolves and validates every executable/launcher/data-home prerequisite before running `TestLiveKimiCapability`, `TestLiveZCodeCapability`, and `TestLiveAgyCapability`; each test uses the production runtime definition, credential/native-home namespace, immutable fixture CWD, native prompt transport, bounded lifecycle/output parser, direct-execution receipts, terminal drain, and protected-state checks. The former `live_e2e_test.go` workflow was no longer executable release authority at that stage.
...
- [x] Reopen release readiness after confirming that G011 built but never executed the release Mulgae binary and excluded retained live negative tests. SOT 1.14.0 and D-060 make the gap release-blocking.
...
- [x] Execute the exact release Mulgae binary against actual Kimi, ZCode, and AGY through Config v1 init/config/doctor, six-role root review, schema-valid P2 publication, diagnostics, cleanup, followup, delta, exact rerun, and recomposed rerun. Evidence: `TestE2EActualProvidersProductionWorkflow` passed through the Makefile-built v1.14 candidate in 1042.036 seconds with all six initial primary assignments and all four child workflows.
- [x] Restore fail-closed prerequisite, retry-authority, assignment, qualification-health, artifact-URI, terminal-process, and child-identity harness coverage removed with `live_e2e_test.go`. Evidence: every `TestLive*` harness case passed in the same non-skipping `make test-e2e`; architecture checks require the file, exact binary environment, child commands, bounded two-attempt policy, and absence of the obsolete process-overlap predicate.
...
## G013 Login-Recovering Release Gate Order

- [x] Reproduce an expired Kimi session as `TestLiveKimiCapability` failing with typed `provider login required` before any exact-binary workflow test starts.
- [x] Run the exact release Mulgae root/child workflow before the standalone family capability suite so production qualification can perform its one bounded Kimi-native login, reconstruct a fresh credential namespace, and retry qualification once.
- [x] Retain all three capability certifications as mandatory after the workflow; do not ignore, skip, or downgrade a remaining authentication or capability failure. Preserve the private E2E project on either workflow or capability failure.
- [x] Bind the order in `TestMakefileContract` and retain the existing unit tests that prohibit non-Kimi login, bound recovery to one attempt, and require the first namespace to drain before login.
- [x] Verify the original expired-session condition end to end: the exact workflow passes with Kimi selected as a completed primary, all restored `TestLive*` workflow harness cases run, and the subsequent Kimi, ZCode, and AGY capability certifications pass in the same `make test-e2e` invocation.
- [x] Require one exact final committed-tree `make test` PASS and a clean working tree before recording G013 as `RELEASE_READY`.
...
- [x] Implement the anchored `.mulgae/diagnostics/<session>/<run>` filesystem store with serialized JSONL, atomic status replacement, separated scanned raw streams, caps, mandatory tail reserve, durable sync, recovery, and exactly-once finalize. Evidence: `ea7b709`, `55b531f`, and focused diagnostic store tests.
