...
func TestParseRejectsJSONOptionsOutsideFrozenRequestVariants(t *testing.T) {
	for _, arguments := range [][]string{
		{"config", "--project-config", ".mulgae.yaml", "--ref", strings.Repeat("a", 40), "--output", "json"},
...
func TestParseRejectsMalformedInput(t *testing.T) {
	tests := []struct {
		...
		{name: "mutable project ref", arguments: []string{"config", "--ref", "refs/heads/main", "--project-config", ".mulgae.yaml"}},
		{name: "project config without expected commit", arguments: []string{"config", "--project-config", ".mulgae.yaml"}},
		...
		{name: "schema invalid ID", arguments: []string{"schema", "show", "urn:mulgae:schema:broken"}},
...
func TestParseCLIExamplesAndCommandSurfaceGoldens(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "agent instructions findings", arguments: []string{"findings", "--run", testRunID, "--severity", "low", "--output", "json"}, command: app.CommandFindings},
