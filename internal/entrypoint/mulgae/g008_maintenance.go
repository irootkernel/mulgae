...
func (reader p2ExportProjectionReader) ReadCommittedProjection(_ context.Context, source appexport.ExportSource) (appexport.VerifiedSourceProjection, error) {
	...
	projection := appexport.VerifiedSourceProjection{
		...
		SchemaVersions:  []string{"mulgae-run-manifest.v2", "mulgae-review-artifact.v3"},
		Review:          appexport.Review{SchemaVersion: "mulgae-review-artifact.v3", ContentVerdict: string(committed.ContentVerdict()), CoverageStatus: string(committed.CoverageStatus())},
		Run:             appexport.Run{SchemaVersion: "mulgae-run-manifest.v2", State: string(committed.RunState())},
