...
func (reader p2ExportProjectionReader) ReadCommittedProjection(_ context.Context, source appexport.ExportSource) (appexport.VerifiedSourceProjection, error) {
	...
	projection := appexport.VerifiedSourceProjection{
		...
		SchemaVersions:  []string{"kar-run-manifest.v2", "kar-review-artifact.v3"},
		Review:          appexport.Review{SchemaVersion: "kar-review-artifact.v3", ContentVerdict: string(committed.ContentVerdict()), CoverageStatus: string(committed.CoverageStatus())},
