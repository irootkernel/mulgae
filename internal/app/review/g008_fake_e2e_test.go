...
func TestG008ProviderRuntimeCapturesRepairArtifactsDeterministically(t *testing.T) {
	...
	valid := []byte(`{"schema_version":"kar-provider-review-output.v2","summary":"No findings.","completeness":"complete","limitations":[],"findings":[]}`)
	...
	budgets := make([]review.RoleBudget, 0, len(domain.FixedRoleOrder()))
