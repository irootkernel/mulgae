...
func TestFollowupValidatorInjectsTrustedEvidenceForEveryResolution(t *testing.T) {
	for _, resolution := range []string{"resolved", "partially_resolved", "still_open", "unclear"} {
		t.Run(resolution, func(t *testing.T) {
			...
			raw := []byte(`{"schema_version":"mulgae-provider-followup-output.v1","summary":"summary","resolution":"` + resolution + `","rationale":"rationale","evidence":[{"current":{"path":"a.go","line_start":1,"line_end":1,"side":"head","quote":"x"}}],"new_findings":[],"limitations":[]}`)
