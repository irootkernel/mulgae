...
func generate() error {
	...
	if err := replaceSchemaMatrix(filepath.Join(assets, "schemas", "mulgae-command-result.v3.schema.json"), specs); err != nil {
		return err
	}
	if err := replaceSchemaOutcomeContract(filepath.Join(assets, "schemas", "mulgae-command-result.v3.schema.json"), specs); err != nil {
		return err
	}
	if err := replaceSchemaDiscoveryContract(filepath.Join(assets, "schemas", "mulgae-command-result.v3.schema.json"), discoverySpecs); err != nil {
		return err
	}
	return nil
}
